...
-- which has exactly $k+4=2 l+4$ elements.
      use Icc (-(l + 2)) (-1) ∪ Icc 1 (l + 2)
      use by rw [card_union_of_disjoint sorry]; simp; omega
...
-- Therefore, we consider only $1 \leqslant i \leqslant l+2$.
      clear S
      wlog hi' : i ∈ Icc 1 (l + 2)
      . specialize this k hk l hl (-i) (by simp at hi ⊢; omega) (by simp at hi hi' ⊢; omega)
...
-- and the numbers $i+1$ and -1 .
      . use (Icc (-(l + 1)) (-2) \ {-i}) ∪ (Icc 2 (l + 1) \ {i}) ∪ {i + 1, -1}
        use by
          intro x hx
          simp at hx ⊢
          omega
        use by
          rw [card_union_of_disjoint sorry]
          rw [card_union_of_disjoint sorry]
          rw [card_sdiff sorry]
          rw [card_sdiff sorry]
          rw [card_pair sorry]
...
-- and the numbers $l+1$ and 1 .
      . subst hi2
        ...
        rw [sum_union sorry]
        rw [sum_union sorry]
        have c1 : ∑ x ∈ Icc (-l) (-2), x = ∑ x ∈ Icc 2 l, -x := by
          apply sum_nbij (-.)
          . simp; omega
          . sorry
          . sorry
