...
theorem other_24328
  ...
  generalize h_1990 : 1990 = _1990
  generalize h_700 : 700 = _700

-- Since $2^{1990}<8^{700}<10^{700}$,
  have h1 : 2 ^ _1990 < 10 ^ _700 := by norm_num [←h_1990, ←h_700]

-- we have $f_{1}\left(2^{1990}\right)<(700 \cdot 9)^{2}<4 \cdot 10^{7}$.
  replace h1 : f^[1] (2 ^ _1990) < 4 * 10 ^ 7 := calc
    _ < (_700 * 9) ^ 2 := by
      rw [iterate_one, f_def]
      apply Nat.pow_lt_pow_left ?_ (by norm_num)
      calc
        _ ≤ (∑ i ∈ Ico 0 _700, (2 ^ _1990) / 10 ^ i % 10) := by
          apply sum_le_sum_of_subset
          suffices ⌊logb 10 (2 ^ _1990)⌋₊ < _700 from by
        ...
        _ < (∑ i ∈ Ico 0 _700, 9) := by
          ...
          . intro i hi; omega
          . use 0
            suffices 2 ^ _1990 % 10 < 9 from by simpa [←h_700]
            rw [←h_1990]
            omega
        _ = _ := by simp
    _ < _ := by norm_num [←h_700]

-- We then have $f_{2}\left(2^{1990}\right)<(3+9 \cdot 7)^{2}<4900$
  have h2 : f^[2] (2 ^ _1990) < 4900 := calc
    _ < (3 + 9 * 7) ^ 2 := by
      rw [iterate_succ_apply', f_def]
      apply Nat.pow_lt_pow_left ?_ (by norm_num)
      calc
        _ ≤ (∑ i ∈ Ico 0 8, (f^[1] (2 ^ _1990)) / 10 ^ i % 10) := by
          apply sum_le_sum_of_subset
          suffices ⌊logb 10 (f^[1] (2 ^ _1990))⌋₊ < 8 from by
            intro x hx; simp at this hx ⊢; omega
          rw [Nat.floor_lt]
          ...
          . apply logb_nonneg
            ...
            . norm_cast
              sorry
        _ = (∑ i ∈ Ico 0 7, (f^[1] (2 ^ _1990)) / 10 ^ i % 10) + (f^[1] (2 ^ _1990)) / 10 ^ 7 % 10 := by
          have : Ico 0 8 = Ico 0 7 ∪ {7} := by ext x; simp; omega
          rw [this, sum_union (by simp)]
          simp
        _ < (∑ i ∈ Ico 0 7, 9) + 3 := by
          suffices ∑ i ∈ Ico 0 7, f^[1] (2 ^ _1990) / 10 ^ i % 10 < (∑ i ∈ Ico 0 7, 9)
              ∧ f^[1] (2 ^ _1990) / 10 ^ 7 % 10 ≤ 3 from by omega
          split_ands
          . apply sum_lt_sum
            . intro i hi; omega
            . sorry
...
-- and finally $f_{3}\left(2^{1990}\right)<(4+9 \cdot 3)^{2}=31^{2}$.
  have h3 : f^[3] (2 ^ _1990) < 31 ^ 2 := calc
    _ < 31 ^ 2 := by
      rw [iterate_succ_apply', f_def]
      apply Nat.pow_lt_pow_left ?_ (by norm_num)
      calc
        _ ≤ (∑ i ∈ Ico 0 4, (f^[2] (2 ^ _1990)) / 10 ^ i % 10) := by
          apply sum_le_sum_of_subset
          suffices ⌊logb 10 (f^[2] (2 ^ _1990))⌋₊ < 4 from by
            intro x hx; simp at this hx ⊢; omega
          rw [Nat.floor_lt]
          . rw [logb_lt_iff_lt_rpow (by norm_num)]
            . norm_cast; omega
          ...
          . apply logb_nonneg
            ...
            . norm_cast
              sorry
        _ =   (f^[2] (2 ^ _1990)) / 10 ^ 0 % 10
            + (f^[2] (2 ^ _1990)) / 10 ^ 1 % 10
            + (f^[2] (2 ^ _1990)) / 10 ^ 2 % 10
            + (f^[2] (2 ^ _1990)) / 10 ^ 3 % 10 := by
          have : Ico 0 4 = {0, 1, 2, 3} := by ext x; simp; omega
          simp [this]; omega
        _ ≤ 30 := by
          have : (f^[2] (2 ^ _1990) / 10 ^ 3 % 10 = 4 ∧ f^[2] (2 ^ _1990) / 10 ^ 2 % 10 ≤ 8)
                  ∨ (f^[2] (2 ^ _1990) / 10 ^ 3 % 10 ≤ 3 ∧ f^[2] (2 ^ _1990) / 10 ^ 2 % 10 ≤ 9) := by
                  omega
          omega
        _ < _ := by norm_num
    _ = _ := by norm_num
...
-- Since $2^{6} \equiv 1(\bmod 9)$, we have $2^{1990} \equiv 2^{4} \equiv 7$
  have h5 : 2 ^ _1990 % 9 = 7 := by norm_num [←h_1990]

-- It follows that $f_{1}\left(2^{1990}\right) \equiv 7^{2} \equiv 4$
  have h6 : f^[1] (2 ^ _1990) % 9 = 4 := by
    specialize h4 0 (2 ^ _1990)
    rw [Nat.pow_mod] at h4
    simpa [h5] using h4

-- and $f_{2}\left(2^{1990}\right) \equiv 4^{2} \equiv 7$.
  have h7 : f^[2] (2 ^ _1990) % 9 = 7 := by
    specialize h4 1 (2 ^ _1990)
    rw [Nat.pow_mod] at h4
    simp at h6
    simpa [h6] using h4
...
-- and $f_{2 k+1}\left(2^{1990}\right) \equiv 4$ for all integer $k>0$.
  have h8 : ∀ k, f^[2 * k] (2 ^ _1990) % 9 = 7 := by
    sorry
  have h9 : ∀ k, f^[2 * k + 1] (2 ^ _1990) % 9 = 7 := by
    sorry
...
-- hence $f_{3}\left(2^{1990}\right) \in\{49,256,625\}$.
  have h14 : f^[3] (2 ^ _1990) = 49 ∨ f^[3] (2 ^ _1990) = 256 ∨ f^[3] (2 ^ _1990) = 625 := by
    rw [h10]
    rcases h13 with h13 | h13 | h13 <;> simp [h13]

-- It follows that $f_{4}\left(2^{1990}\right)=169,
  have h15 : f^[4] (2 ^ _1990) = 169 := by
    rw [iterate_succ_apply']
